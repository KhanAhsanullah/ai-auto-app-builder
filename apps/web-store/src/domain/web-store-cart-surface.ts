...
import { WebStoreCartUnavailableException } from '../errors.js';
import type { ResolvedWebStoreShell } from '../types.js';

export interface WebStoreCartBinding {
  cart: CartModule;
  /** ISO currency used when getOrCreate does not pass currency. */
...
/**
 * Storefront cart access: feature-flag gated + tenant-scoped.
 */
export class WebStoreCartSurface {
  private readonly flags: FeatureFlagEvaluator;
  private readonly tenantId: string;

  constructor(
    private readonly shell: ResolvedWebStoreShell,
    private readonly binding: WebStoreCartBinding | undefined,
  ) {
  ...
  private requireCart(): WebStoreCartBinding {
    if (!this.binding) {
      throw new WebStoreCartUnavailableException(
        'Cart module is not wired. Pass cart to createWebStore({ cart }).',
      );
    }
