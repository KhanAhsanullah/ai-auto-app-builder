import type { BoomLaunchVertical } from '../types.js';

/** Platform-owned demo product row served to hosts. */
...
/** Persisted catalog for one tenant. */
export interface TenantCatalogRecord {
  tenantId: string;
...
const SEEDS: Record<BoomLaunchVertical, readonly SeedProduct[]> = {
  grocery: [
    {
      slug: 'atta',
      name: 'Atta Flour',
...
/** Build a vertical demo catalog record for a tenant. */
export function buildVerticalCatalogRecord(input: {
  tenantId: string;
  vertical: BoomLaunchVertical;
  updatedAt: string;
}): TenantCatalogRecord {
  const products: PlatformCatalogProduct[] = SEEDS[input.vertical].map((seed) => ({
    id: `${input.tenantId}:${seed.slug}`,
    slug: seed.slug,
    name: seed.name,
    sku: seed.sku,
    title: seed.title,
